...
int main(int argc, const char * argv[])
{
	...
	// promt the user for a decimal number that will be used as page size
	while(!ok)
	{
		printf("\nChoose a number between 0 and 4096 for the page size: ");
		scanf("%d", &number);
		// error checking for number between 0 and 1000
		if(number < 1 || number > 4095)
		{
			printf("This number needs to be between 0 and 4096\n");
		}
		// print number
		else
		{
	...
	// display number as a 16 bit binary number
	printf("The number in binary is: ");
	convertPrintBinary(number, &bits[0]);
	
	// display the number as a 16 bit hexidecimal number
	printf("The page size as a hexidecimal is: 0x%08X\n", number);
